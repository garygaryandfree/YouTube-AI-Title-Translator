...
v7.4 修复 YouTube 播放页切换视频时，中文标题区域偶尔继续显示上一个视频标题的问题。插件现在会识别 YouTube 单页应用复用标题节点的情况，并在原标题变化时清理旧翻译、重新渲染当前视频标题。

| 更新点 | 说明 |
|--------|------|
| 播放页标题同步 | 标题节点文字变化时自动清理旧翻译框，避免显示上一个视频的中文标题 |
| 迟到结果保护 | 异步翻译返回时只更新仍在页面上的翻译框，避免旧请求污染新页面 |
| 回归验证 | 增加本地模拟验证，覆盖 YouTube 播放页复用同一个标题节点的场景 |
| 版本更新 | manifest、弹窗版本标识与 release 包同步更新到 v7.4 |
| v7.3 隐私政策 | 新增公开隐私政策，说明数据处理、本地存储、第三方 AI 服务和限定用途 |
| v7.3 安全传输 | 自定义端点必须使用 HTTPS，本机 localhost 调试接口除外 |
| v7.3 权限说明 | 明确 YouTube 页面访问、storage 和可选主机权限的用途 |
| v7.3 配置简化 | 删除“高级设置”区，减少重复入口 |
| v7.3 服务商扩展 | 新增 Claude、智谱、Kimi，整体改为 4 列两行布局 |
| v7.3 模型更新 | 更新 OpenAI、Gemini、Claude、DeepSeek、MiniMax、智谱、Kimi 的推荐模型 |
| v7.3 手动模型 | 所有预设服务商都支持“其他（手动输入模型名）” |
| v7.3 Claude 支持 | Background Service Worker 增加 Anthropic Messages API 调用格式 |
| v7.3 品牌图标 | Claude、智谱/Z.ai、Kimi 使用真实 logo 资源 |
| v7.3 视觉调整 | 主操作色和“支持开发者”按钮统一为红色系 |
