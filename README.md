...
v7.3 聚焦 Chrome Web Store 隐私与权限合规：补充隐私政策、限定用途说明，并限制自定义端点的安全传输方式。

| 更新点 | 说明 |
|--------|------|
| 隐私政策 | 新增公开隐私政策，说明数据处理、本地存储、第三方 AI 服务和限定用途 |
| 安全传输 | 自定义端点必须使用 HTTPS，本机 localhost 调试接口除外 |
| 权限说明 | 明确 YouTube 页面访问、storage 和可选主机权限的用途 |
